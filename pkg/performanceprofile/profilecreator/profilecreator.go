...
// if core IDs numbering was found different on same NUMA cells of different compute nodes, the function will return true
// so this be printed out as a warning to the user later
func EnsureNodesHaveTheSameHardware(nodeHandlers []*GHWHandler) (bool, error) {
	differentCoreIDsFound := false

	if len(nodeHandlers) < 1 {
		return differentCoreIDsFound, fmt.Errorf("no suitable nodes to compare")
	}

	firstHandle := nodeHandlers[0]
	firstTopology, err := firstHandle.SortedTopology()
	if err != nil {
		return differentCoreIDsFound, fmt.Errorf("can't obtain Topology info from GHW snapshot for %s: %v", firstHandle.Node.GetName(), err)
	}
	for _, handle := range nodeHandlers[1:] {
		if err != nil {
			return differentCoreIDsFound, fmt.Errorf("can't obtain GHW snapshot handle for %s: %v", handle.Node.GetName(), err)
		}

		topology, err := handle.SortedTopology()
		if err != nil {
			return differentCoreIDsFound, fmt.Errorf("can't obtain Topology info from GHW snapshot for %s: %v", handle.Node.GetName(), err)
		}
		found, err := ensureSameTopology(firstTopology, topology)
		if err != nil {
			return differentCoreIDsFound, fmt.Errorf("nodes %s and %s have different topology: %v", firstHandle.Node.GetName(), handle.Node.GetName(), err)
		}
		if found {
			differentCoreIDsFound = true
		}
	}

	return differentCoreIDsFound, nil
}

func ensureSameTopology(topology1, topology2 *topology.Info) (bool, error) {
	...
	if topology1.Architecture != topology2.Architecture {
		return differentCoreIDsFound, fmt.Errorf("the architecture is different: %v vs %v", topology1.Architecture, topology2.Architecture)
	}

	if len(topology1.Nodes) != len(topology2.Nodes) {
		return differentCoreIDsFound, fmt.Errorf("the number of NUMA nodes differ: %v vs %v", len(topology1.Nodes), len(topology2.Nodes))
	}

	for i, node1 := range topology1.Nodes {
		node2 := topology2.Nodes[i]
		if node1.ID != node2.ID {
			return differentCoreIDsFound, fmt.Errorf("the NUMA node ids differ: %v vs %v", node1.ID, node2.ID)
		}

		cores1 := node1.Cores
		cores2 := node2.Cores
		if len(cores1) != len(cores2) {
			return differentCoreIDsFound, fmt.Errorf("the number of CPU cores in NUMA node %d differ: %v vs %v",
		...
		for j, core1 := range cores1 {
			...
			if core1.NumThreads != cores2[j].NumThreads {
				return differentCoreIDsFound, fmt.Errorf("number of threads for CPU %d in NUMA node %d differs: %d vs %d", core1.ID, node1.ID, core1.NumThreads, cores2[j].NumThreads)
			}
			if !reflect.DeepEqual(core1.LogicalProcessors, cores2[j].LogicalProcessors) {
				return differentCoreIDsFound, fmt.Errorf("logical processors for CPU %d in NUMA node %d differs: %d vs %d", core1.ID, node1.ID, core1.LogicalProcessors, cores2[j].LogicalProcessors)
			}
		}
	}
	return differentCoreIDsFound, nil
